...
This guide helps you get up and running with one or more EverQuest clients on Linux using Wine, while giving you a practical understanding of the pieces involved so you can scale (boxing) and troubleshoot with confidence. Principle goals are:
...
5. Finally you will need EverQuest **login credentials** for each EverQuest instance that you intend to run simultaneously. EQAscendant (at the time of this writing) allows 3 clients out in the world killing stuff, and an additional client in the Bazaar selling your **Phat Loots!**. To get login credentials:
   ...
   2. Login to your EQEmulator account and:
      ...
      - Click Create Account, and follow the prompts to create loginserver credentials.
      - I think they have a limit on how many Loginserver accounts you can create in a day. 
      - You can use any existing credentials that you may have made for other EMUs. They are not EMU exclusive.
...
- Download the matching Wine Mono installer: wine-mono-11.0.0-x86.msi 
...
##### /BEGIN REPEATABLE PROCESS

**Step 1:** Create the prefix directory: **(eq1 x 1)**

```bash
mkdir -p ~/Games/EQAscendant/eq1
```

**Step 2:** Initialize the Wine prefix: **(eq1 x 1)**
...
**Step 3:** Install DXVK into the eq1 prefix: **(eq1 x 1)**
...
1. Create the EverQuest install directory inside the prefix:  **(eq1 x 2)**

```bash
mkdir -p ~/Games/EQAscendant/eq1/drive_c/Program\ Files/eq1
```

2. Copy the game files: **(eq1 x 2)**
...
In this Step, we explicitly finalize first‑launch display containment before starting the game. This ensures a predictable, non‑disruptive first run. 

1. Enable Wine Virtual Desktop: Open Wine configuration for this prefix: **(eq1 x 1)**
...
- Graphics tab: 
  ...
  - Set the desktop size to 1600 x 900
...
**Step 7:** Launch the EQAscendant patcher, which will in turn launch EverQuest:

1. Change to the EverQuest install directory: **(eq1 x 2)**

```bash
cd ~/Games/EQAscendant/eq1/drive_c/Program\ Files/eq1
```

2. Launch the EQAscendant patcher: **(eq1 x 1)**
...
**Step 8:** Create a desktop launcher (eq1). 

Create a launcher so you can start EverQuest conveniently. 
...
2. Create the icons directory (if it doesn't exist).

```bash
mkdir -p ~/.local/share/icons
```

3. Copy the patcher icon to  ~/.local/share/icons. **(eq1 x 3)**
...
4. Create the launcher .desktop file: **(eq1 x 8)**
...
5. Refresh desktop application database:
...
6. Copy the launcher from /applications to /Desktop:
...
1. Paste the code as is from the guide to terminal, but before you press Enter: 
   - use the Left-arrow key to scroll non-destructively through the command and change each occurrence of eq1 to eq2. Use backspace or del to remove the 1 depending on where your cursor is, then type 2
   - After making the changes you can press Enter from anywhere in the command to execute it
2. (**Preferred method for safety**) Paste the code as is from the guide into a text editor. I like kate (sudo apt install kate) for its notepad/notepad++ like interface. 
   - Edit each occurrence of eq1 to eq2 in the text editor. 
   - Copy the result and paste in terminal, then press Enter

3) **Don't do any of that.** Be lazy and safe and just copy and paste from my prefabs for eq2 - eq4 installs. They are found after the Appendix below.
...
**Step 1:** Create the prefix directory: 

```bash
mkdir -p ~/Games/EQAscendant/eq2
```

**Step 2:** Initialize the Wine prefix:
...
**Step 3:** Install DXVK into the eq2 prefix: 

```bash
WINEPREFIX=~/Games/EQAscendant/eq2 winetricks dxvk
```

**Step 4:** Copy EverQuest files into the eq2 prefix 
 ...
 **Verify the file copy paste succeeded:** Confirm the copy completed without errors. Browse to ~/Games/EQAscendant/eq1/drive_c/Program\ Files/eq2/ using your file manager or terminal. Confirm the destination contains many files and subdirectories.
...
1. Change to the EverQuest install directory:

```bash
cd ~/Games/EQAscendant/eq2/drive_c/Program\ Files/eq2
```

2. Launch the EQAscendant patcher:
...
**Step 8:** Create a desktop launcher (eq2). 
...
3. Copy the patcher icon to  ~/.local/share/icons.

```bash
cp ~/Games/EQAscendant/eq2/drive_c/Program\ Files/eq2/eqemupatcher.png 
~/.local/share/icons/eqascendant-eq2.png
```

4. Create the launcher .desktop file:
